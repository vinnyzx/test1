<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
    ...
    <div>
        <label class="text-sm font-semibold text-slate-700 dark:text-slate-200">Logo URL</label>
        <input type="url" name="logo_url" value="{{ old('logo_url', $brand->logo_url ?? '') }}" class="mt-1.5 w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 focus:border-primary focus:ring-primary" placeholder="https://..." />
        @error('logo_url')
        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        @enderror
    </div>
